...
void SimpleComputePipeline::pushConstant(
    VkCommandBuffer buf, IntegralPushContant const *push_const) {
  vkCmdPushConstants(buf, pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
                     sizeof(IntegralPushContant), push_const);
}

void SimpleComputePipeline::doIteration(VkCommandBuffer buf,
                                        VkDescriptorSet const *descriptorSet,
                                        SyncObjects const &objs, size_t iter,
                                        IntegralPushContant const *pConst,
                                        uint32_t const sizes[3]) {
  ...
  pushConstant(buf, pConst);
  vkCmdDispatch(buf, sizes[0], sizes[1], 1);
